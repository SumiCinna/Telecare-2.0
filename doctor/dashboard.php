...
// ── Stats ──
$patient_count  = $conn->query("SELECT COUNT(*) c FROM patient_doctors WHERE doctor_id=$doctor_id")->fetch_assoc()['c'];
$today_appts    = $conn->query("SELECT COUNT(*) c FROM appointments WHERE doctor_id=$doctor_id AND appointment_date=CURDATE() AND status IN ('Pending','Confirmed')")->fetch_assoc()['c'];
$pending_count  = $conn->query("SELECT COUNT(*) c FROM appointments WHERE doctor_id=$doctor_id AND status='Pending'")->fetch_assoc()['c'];
