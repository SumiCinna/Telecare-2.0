...
<style>
  ...
  .btn-edit-summary{background:rgba(245,158,11,0.1);color:#b45309;border:1px solid rgba(245,158,11,0.25);border-radius:6px;padding:0.3rem 0.7rem;font-size:0.73rem;font-weight:700;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:0.3rem;margin-top:0.5rem;}
  .btn-edit-summary:hover{background:rgba(245,158,11,0.2);}
  .btn-edit-reviewed{background:rgba(34,197,94,0.09);color:#15803d;border:1px solid rgba(34,197,94,0.25);border-radius:6px;padding:0.3rem 0.7rem;font-size:0.73rem;font-weight:700;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:0.3rem;margin-top:0.5rem;}
  .btn-edit-reviewed:hover{background:rgba(34,197,94,0.18);}
  ...
  .edit-summary-header {
    background: linear-gradient(135deg,#244441,#1a3533);
  ...
  .edit-summary-body { padding: 1.2rem 1.4rem; }
  .summary-textarea {
    width: 100%;
    min-height: 420px;
  ...
  .btn-save-summary:disabled { opacity: 0.6; cursor: not-allowed; }
  .btn-cancel-edit {
...
<div class="page">
  ...
  <?php endif;
    ...
    $hasDraftSummary = !empty(trim((string)($a['consultation_summary'] ?? '')));
    $summaryEdited   = !empty($a['summary_edited']) && (int)$a['summary_edited'] === 1;
    ...
    // Encode summary for JS (safe)
    $summary_for_js = htmlspecialchars($a['consultation_summary'] ?? '', ENT_QUOTES, 'UTF-8');
  ?>
  ...
  <div class="appt-card <?= $cardClass ?>">
    ...
    <div class="appt-time-strip">
      ...
      <span style="margin-left:auto;display:flex;align-items:center;gap:0.5rem;">
        ...
        <?php if ($summaryEdited): ?><span class="reviewed-badge">✓ Summary Reviewed</span><?php endif; ?>
      </span>
    </div>
    ...
    <div class="appt-card-body">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:0.4rem;align-items:center;">
        ...
        <?php if (($status === 'Completed' || $status === 'Confirmed') && $hasSummary): ?>
          ...
          <!-- Edit Summary button -->
          <button class="btn-edit-summary"
            onclick="openEditSummary(<?= $a['id'] ?>, <?= $summaryEdited ? '1' : '0' ?>)"
            data-appt-id="<?= $a['id'] ?>"
            data-summary="<?= $summary_for_js ?>">
            ✏️ <?= $summaryEdited ? 'Re-edit Summary' : 'Edit & Confirm' ?>
          </button>

        <?php elseif (($status === 'Completed' || $status === 'Confirmed') && $hasDraftSummary): ?>
          <!-- Has draft but no PDF yet — edit directly -->
          <button class="btn-edit-summary"
            onclick="openEditSummary(<?= $a['id'] ?>, 0)"
            data-appt-id="<?= $a['id'] ?>"
            data-summary="<?= $summary_for_js ?>">
            ✏️ Review & Confirm Summary
          </button>
...
<div class="modal-overlay" id="modal-edit-summary" onclick="if(event.target===this)closeEditSummary()">
  <div class="modal" style="width:100%;max-width:720px;max-height:92vh;overflow-y:auto;border-radius:20px;background:#fff;padding:0;">
    ...
    <div class="edit-summary-header">
      <div>
        <div style="font-size:0.7rem;font-weight:800;letter-spacing:0.1em;text-transform:uppercase;opacity:0.7;margin-bottom:0.25rem;">TELE-CARE</div>
        <div style="font-size:1.15rem;font-weight:800;">Review & Edit Consultation Summary</div>
        <div style="font-size:0.73rem;opacity:0.75;margin-top:0.15rem;">Make any corrections before confirming. This will update the PDF sent to the patient.</div>
      </div>
      <button onclick="closeEditSummary()" style="background:rgba(255,255,255,0.15);border:none;color:#fff;width:32px;height:32px;border-radius:50%;cursor:pointer;font-size:1rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;">✕</button>
    </div>

    <div class="edit-summary-body">
      <!-- Tips -->
      <div style="background:rgba(34,197,94,0.06);border:1px solid rgba(34,197,94,0.2);border-radius:12px;padding:0.7rem 0.9rem;margin-bottom:0.9rem;font-size:0.75rem;color:#15803d;line-height:1.6;">
        <strong>Tips:</strong> Keep the numbered section headers (e.g. <code>1. Chief Complaint</code>) intact. Edit only the content below each header.
        After saving, the updated PDF will be available to the patient in their visit history.
      </div>
      ...
      <textarea
        ...
        placeholder="Loading summary..."></textarea>

      <div class="edit-summary-hint">
        ⚠️ This summary is AI-generated. As the attending physician, please verify accuracy before confirming.
        Saving will mark this summary as <strong>doctor-reviewed</strong> and regenerate the patient's PDF.
      </div>
    </div>

    <div class="edit-summary-footer">
      <button class="btn-cancel-edit" onclick="closeEditSummary()">Cancel</button>
      <button class="btn-save-summary" id="btn-save-summary" onclick="saveSummary()">
        <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
        Save & Confirm Summary
      </button>
    </div>
...
function showToast(msg, color) {
  const t = document.getElementById('toast-js');
  t.textContent = msg;
...
function openEditSummary(apptId, isEdited) {
  // Find the summary text from the button's data attribute
  const btn = document.querySelector(`[data-appt-id="${apptId}"][data-summary]`);
  const summary = btn ? btn.dataset.summary : '';

  document.getElementById('edit-appt-id').value = apptId;
  document.getElementById('summary-edit-textarea').value = summary;
  document.getElementById('modal-edit-summary').classList.add('active');
  document.body.style.overflow = 'hidden';

  // Focus textarea
  setTimeout(() => document.getElementById('summary-edit-textarea').focus(), 200);
}

function closeEditSummary() {
  document.getElementById('modal-edit-summary').classList.remove('active');
  document.body.style.overflow = '';
}

async function saveSummary() {
  const apptId  = document.getElementById('edit-appt-id').value;
  const summary = document.getElementById('summary-edit-textarea').value.trim();

  if (!summary) {
    showToast('Summary cannot be empty.', '#ef4444');
    return;
  }
  ...
  try {
    ...
    if (data.status === 'success') {
      showToast('✓ Summary saved & PDF updated! Visible in patient visits.', '#16a34a');
      closeEditSummary();

      // Update the button's data-summary with new text so re-open works
      const editBtns = document.querySelectorAll(`[data-appt-id="${apptId}"][data-summary]`);
      editBtns.forEach(b => b.dataset.summary = summary);

      // Refresh card after short delay to show "✓ Summary Reviewed" badge
      setTimeout(() => location.reload(), 1800);
    } else {
      showToast('Error: ' + (data.message || 'Save failed'), '#ef4444');
    }
  } catch (err) {
    showToast('Network error. Please try again.', '#ef4444');
  } finally {
    btn.disabled = false;
    btn.innerHTML = '<svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Save & Confirm Summary';
  }
}
...
<div class="card">
  <div class="hdr">
    <div style="font-size:.7rem;font-weight:800;letter-spacing:.12em;text-transform:uppercase;opacity:.7;margin-bottom:.4rem;">🏥 Tele-Care</div>
    <div style="font-size:1.5rem;font-weight:800;margin-bottom:.2rem;">Payment Receipt</div>
    <div style="font-size:.75rem;opacity:.75;">Official Consultation Receipt</div>
  </div>
  <div class="body">
    <div class="success-bar">
      ...
      <div>
        <div style="font-weight:700;font-size:.88rem;color:#15803d;">Payment Successful</div>
        <div style="font-size:.72rem;color:#16a34a;opacity:.85;">${paidAt}</div>
      </div>
    </div>
    ...
    <div class="amount-box">
      <div style="font-size:.68rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#7a9a97;margin-bottom:.3rem;">Total Amount Paid</div>
      <div class="amount-val">${amount}</div>
      <div><span class="paid-badge">✓ PAID</span></div>
    </div>
    <div class="meta-row"><span style="color:#7a9a97;font-weight:600;">Payment via</span><span style="color:#244441;font-weight:700;">PayMongo (GCash / Card)</span></div>
    <div class="meta-row" style="margin-bottom:.5rem"><span style="color:#7a9a97;font-weight:600;">Appointment ID</span><span style="color:#244441;font-weight:700;">${apptId}</span></div>
  </div>
