...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $doctor && !$error) {
    ...
    if (strlen($password) < 8) {
        ...
        $stmt = $conn->prepare("UPDATE doctors SET password=?, bio=?, consultation_fee=?, languages_spoken=?, clinic_name=?, phone_number=?, profile_photo=COALESCE(?,profile_photo), consent_signed=?, invite_token=NULL, setup_complete=1, status='active', is_available=1 WHERE id=?");
        $stmt->bind_param("ssdsssiii", $hashed, $bio, $fee, $langs, $clinic, $phone, $photo_path, $consent, $doctor['id']);
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .left-panel {
      width:40%; background:linear-gradient(160deg,var(--green) 0%,#1a3330 100%);
      display:flex; flex-direction:column; justify-content:center;
      padding:3rem; position:sticky; top:0; height:100vh; overflow:hidden;
    }
    .left-panel::before {
      content:''; position:absolute; inset:0;
      background-image:linear-gradient(rgba(63,130,227,0.08) 1px,transparent 1px),linear-gradient(90deg,rgba(63,130,227,0.08) 1px,transparent 1px);
    ...
    .field-input {
      ...
      color:var(--green); outline:none; transition:border-color 0.2s; background:var(--white);
    }
    ...
    .section-label {
      ...
      color:#9ab0ae; border-bottom:1px solid rgba(36,68,65,0.1); padding-bottom:0.5rem;
    ...
    .alert-success { background:rgba(36,68,65,0.08); border:1px solid rgba(36,68,65,0.2); color:var(--green); border-radius:12px; padding:0.75rem 1rem; font-size:0.86rem; margin-bottom:1.2rem; }
    ...
    .consent-box {
      background:rgba(36,68,65,0.05); border:1px solid rgba(36,68,65,0.1);
      border-radius:14px; padding:1rem 1.2rem; margin:1rem 0;
      font-size:0.82rem; color:#6b8a87; line-height:1.65;
    }
...
<div class="left-panel">
  ...
  <div style="position:relative;z-index:2;">
    <div style="font-family:'Playfair Display',serif;font-size:1.5rem;font-weight:900;color:#fff;letter-spacing:0.04em;">
      TELE<span style="color:var(--red)">-</span>CARE
    </div>
    <div style="margin-top:3rem;">
      <h1 style="font-size:2rem;color:#fff;line-height:1.2;margin-bottom:0.8rem;">
        Complete Your<br/>Doctor Profile
      </h1>
      <p style="color:rgba(255,255,255,0.55);font-size:0.88rem;line-height:1.75;">
        You've been invited to join TELE-CARE. Set your password, fill in your profile, and configure your availability to go live.
      </p>
    </div>
    <?php if($doctor): ?>
...
<div class="right-panel">
  <div class="form-wrap">
    ...
    <?php if($success): ?>
    <div style="text-align:center;padding:3rem 1rem;animation:fadeUp 0.5s ease;">
      ...
      <p style="color:#6b8a87;font-size:0.95rem;line-height:1.7;margin-bottom:2rem;">
        Your profile is complete and your account is now active. You can now log in to the TELE-CARE system.
      </p>
      <a href="../auth/login.php" style="display:inline-flex;align-items:center;gap:0.5rem;background:var(--red);color:#fff;padding:0.9rem 2.5rem;border-radius:50px;font-weight:700;text-decoration:none;box-shadow:0 6px 20px rgba(195,54,67,0.3);">
        Go to Login →
      </a>
    </div>
    ...
    <div style="margin-bottom:2rem;">
      ...
      <p style="color:#6b8a87;font-size:0.88rem;">Fill in everything below. All fields marked * are required.</p>
    </div>
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="form-row">
        <div class="form-field">
          ...
          <input type="tel" name="phone_number" class="field-input" placeholder="09XXXXXXXXX"/>
        </div>
        <div class="form-field">
          <label class="field-label">Clinic / Hospital</label>
          <input type="text" name="clinic_name" class="field-input" placeholder="Clinic name"/>
        </div>
      </div>
      <div class="form-row">
        <div class="form-field">
          <label class="field-label">Consultation Fee (₱)</label>
          <input type="number" name="consultation_fee" class="field-input" placeholder="e.g. 500" min="0" step="0.01"/>
        </div>
        <div class="form-field">
          <label class="field-label">Languages Spoken</label>
          <input type="text" name="languages_spoken" class="field-input" placeholder="e.g. English, Filipino"/>
        </div>
      </div>
      <div class="form-field">
        <label class="field-label">Profile Photo <span style="font-weight:400;text-transform:none;font-size:0.7rem;">(optional)</span></label>
      ...
      <div class="form-field">
        ...
        <textarea name="bio" class="field-input" placeholder="Brief professional background..."></textarea>
      </div>
      ...
      <div id="schedule-rows">
        <div class="schedule-row">
          ...
          <input type="time" name="start_time[]" class="field-input" placeholder="Start"/>
          <input type="time" name="end_time[]"   class="field-input" placeholder="End"/>
          <div></div>
        </div>
      </div>
      ...
      <div style="display:flex;align-items:flex-start;gap:0.7rem;margin-bottom:1.5rem;">
        ...
        <label for="consent" style="font-size:0.85rem;color:var(--green);cursor:pointer;line-height:1.5;">
          I have read and agree to the telehealth consent agreement and platform terms.
        </label>
      </div>
...
<script>
  function togglePw(fid, sid, hid) {
    const f=document.getElementById(fid), s=document.getElementById(sid), h=document.getElementById(hid);
  ...
  document.getElementById('pw2')?.addEventListener('input', function(){
    ...
    else{m.textContent='✗ Does not match';m.style.color='var(--red)';}
  });
