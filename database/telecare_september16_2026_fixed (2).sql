...
DELIMITER ;;
/*!50003 CREATE*/  /*!50003 TRIGGER `trg_doctor_ratings_after_insert` AFTER INSERT ON `doctor_ratings` FOR EACH ROW BEGIN
  UPDATE `doctors` d
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/  /*!50003 TRIGGER `trg_doctor_ratings_after_update` AFTER UPDATE ON `doctor_ratings` FOR EACH ROW BEGIN
  UPDATE `doctors` d
  SET d.rating = (SELECT ROUND(AVG(r.rating), 1) FROM `doctor_ratings` r WHERE r.doctor_id = NEW.doctor_id),
      d.rating_count = (SELECT COUNT(*) FROM `doctor_ratings` r WHERE r.doctor_id = NEW.doctor_id)
  WHERE d.id = NEW.doctor_id;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/  /*!50003 TRIGGER `trg_doctor_ratings_after_delete` AFTER DELETE ON `doctor_ratings` FOR EACH ROW BEGIN
  UPDATE `doctors` d
  SET d.rating = (SELECT COALESCE(ROUND(AVG(r.rating), 1), 0.0) FROM `doctor_ratings` r WHERE r.doctor_id = OLD.doctor_id),
      d.rating_count = (SELECT COUNT(*) FROM `doctor_ratings` r WHERE r.doctor_id = OLD.doctor_id)
  WHERE d.id = OLD.doctor_id;
