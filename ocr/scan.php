...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['doc_file'])) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($file['tmp_name'], $filepath)) {
            ...
            $py_script = realpath(__DIR__ . '/ocr_scan.py');
            $command   = "python \"$py_script\" \"$abs_path\" 2>&1";
            ...
            if ($decoded && $decoded['success']) {
                ...
                $error = 'OCR failed: ' . ($decoded['error'] ?? $output ?? 'Unknown error');
            }
        } else {
  ...
  <div style="margin-bottom:1rem;">
    ...
    <div class="scan-item" onclick="toggleScan(<?= $scan['id'] ?>)">
      <div style="display:flex;align-items:center;gap:0.8rem;">
        ...
        <?php endif; ?>
        <div style="flex:1;">
          <div style="font-weight:700;font-size:0.9rem;"><?= htmlspecialchars($scan['doc_label'] ?: 'Untitled') ?></div>
